...
{ 			 		 
	...
	while(1)
	{	   
		BSP_W25Qx_WriteInPage(pBuffer,WriteAddr,pageremain);
...
{ 
   ...
   W25QXX_BUF=W25QX_BUFFER;	     
	...
	{	
		...
		if(i<secremain)//需要擦除
		{
			BSP_W25Qx_Erase_Block(secpos*4096);//擦除这个扇区
			for(i=0;i<secremain;i++)	   //复制
			{
				W25QXX_BUF[i+secoff]=pBuffer[i];	  
			}
			BSP_W25Qx_WriteNoCheck(W25QXX_BUF,secpos*4096,4096);//写入整个扇区  

		}else BSP_W25Qx_WriteNoCheck(pBuffer,WriteAddr,secremain);//写已经擦除了的,直接写入扇区剩余区间. 				   
